...
class PagesManifestPlugin{apply(compiler){compiler.hooks.emit.tap('NextJsPagesManifest',compilation=>{const{chunks}=compilation;const pages={};for(const chunk of chunks){const result=_constants.ROUTE_NAME_REGEX.exec(chunk.name);if(!result){continue;}const pagePath=result[1];if(!pagePath){continue;}// Write filename, replace any backslashes in path (on windows) with forwardslashes for cross-platform consistency.
