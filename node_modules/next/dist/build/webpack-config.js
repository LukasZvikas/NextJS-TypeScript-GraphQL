...
if(res.match(/node_modules[/\\]webpack/)||res.match(/node_modules[/\\]css-loader/)){return callback();}// styled-jsx has to be transpiled
if(res.match(/node_modules[/\\]styled-jsx/)){return callback();}if(res.match(/node_modules[/\\].*\.js$/)){return callback(undefined,`commonjs ${request}`);}callback();});}]:[// When the serverless target is used all node_modules will be compiled into the output bundles
'amp-toolbox-optimizer'],optimization:Object.assign({checkWasmTypes:false,nodeEnv:false},isServer?{splitChunks:false,minimize:false}:{runtimeChunk:selectivePageBuilding?false:{name:_constants.CLIENT_STATIC_FILES_RUNTIME_WEBPACK},splitChunks:dev?{cacheGroups:{default:false,vendors:false}}:selectivePageBuilding?{cacheGroups:{default:false,vendors:false,react:{name:'commons',chunks:'all',test:/[\\/]node_modules[\\/](react|react-dom)[\\/]/}}}:{chunks:'all',cacheGroups:{default:false,vendors:false,commons:{name:'commons',chunks:'all',minChunks:totalPages>2?totalPages*0.5:2},react:{name:'commons',chunks:'all',test:/[\\/]node_modules[\\/](react|react-dom)[\\/]/}}},minimize:!(dev||debug),minimizer:!(dev||debug)?[new _index2.TerserPlugin({...terserPluginConfig,terserOptions:{safari10:true,...(selectivePageBuilding||config.experimental.terserLoader?{compress:false,mangle:true}:undefined)}})]:undefined},selectivePageBuilding?{providedExports:false,usedExports:false,concatenateModules:false}:undefined),recordsPath:selectivePageBuilding?undefined:_path.default.join(outputPath,'records.json'),context:dir,// Kept as function to be backwards compatible
...
module:{rules:[(selectivePageBuilding||config.experimental.terserLoader)&&!isServer&&!debug&&{test:/\.(js|mjs|jsx)$/,exclude:/\.min\.(js|mjs|jsx)$/,use:{loader:'next-minify-loader',options:{terserOptions:{safari10:true,compress:true,mangle:false}}}},config.experimental.ampBindInitData&&!isServer&&{test:/\.(tsx|ts|js|mjs|jsx)$/,include:[_path.default.join(dir,'data')],use:'next-data-loader'},{test:/\.(tsx|ts|js|mjs|jsx)$/,include:[dir,/next-server[\\/]dist[\\/]lib/,/next[\\/]dist[\\/]client/,/next[\\/]dist[\\/]pages/],exclude:path=>{if(/next-server[\\/]dist[\\/]lib/.test(path)||/next[\\/]dist[\\/]client/.test(path)||/next[\\/]dist[\\/]pages/.test(path)){return false;}return /node_modules/.test(path);},use:defaultLoaders.babel}].filter(Boolean)},plugins:[// This plugin makes sure `output.filename` is used for entry chunks
...
...(dev&&!isServer?{'process.env.__NEXT_DIST_DIR':JSON.stringify(distDir)}:{}),'process.env.__NEXT_EXPERIMENTAL_DEBUG':JSON.stringify(debug),'process.env.__NEXT_EXPORT_TRAILING_SLASH':JSON.stringify(config.experimental.exportTrailingSlash)}),!isServer&&new _reactLoadablePlugin.ReactLoadablePlugin({filename:_constants.REACT_LOADABLE_MANIFEST}),!isServer&&new _nextDropClientPagePlugin.DropClientPage(),new _chunkGraphPlugin.ChunkGraphPlugin(buildId,{dir,distDir,isServer}),...(dev?(()=>{// Even though require.cache is server only we have to clear assets from both compilations
