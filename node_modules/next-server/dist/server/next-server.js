...
class Server {
    ...
    resolveApiRequest(pathname) {
        return require_1.getPagePath(pathname, this.distDir);
    }
    generatePublicRoutes() {
        const routes = [];
        const publicFiles = recursive_readdir_sync_1.recursiveReadDirSync(this.publicDir);
        const serverBuildPath = path_1.join(this.distDir, constants_1.SERVER_DIRECTORY);
    ...
    async renderToHTML(req, res, pathname, query = {}, { amphtml, dataOnly, hasAmp } = {}) {
        try {
            ...
            const html = await this.renderToHTMLWithComponents(req, res, pathname, query, Object.assign({}, this.renderOpts, { amphtml, hasAmp, dataOnly }));
            return html;
        }
        catch (err) {
